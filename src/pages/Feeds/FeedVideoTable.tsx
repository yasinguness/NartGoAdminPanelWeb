...
import {
  ...
  TableRow, TableCell, TableSortLabel, Paper
} from '@mui/material';
...
export default function FeedVideoTable({
  ...
  onStatusChange, sortField, sortDir, onSort, statusConfig
}: Props) {
      ...
      <Table size="small">
        <TableHead>
          <TableRow sx={{ bgcolor: '#fafafa' }}>
            ...
            <TableCell sx={{ fontWeight: 600, fontSize: '0.75rem', color: 'text.secondary' }}>MODERATED AT</TableCell>
          </TableRow>
        </TableHead>
        <TableBody>
          {videos.map((video) => {
              ...
              <TableRow key={video.id} hover onClick={() => onClick(video)} sx={{ cursor: 'pointer', bgcolor: isSelected ? 'rgba(22, 70, 28, 0.03)' : 'transparent' }}>
                ...
                <TableCell>
                  <Stack direction="row" alignItems="center" spacing={1.5}>
                    ...
                    <Typography variant="body2" sx={{ fontWeight: 600, fontSize: '0.8rem' }}>{video.title}</Typography>
                  </Stack>
                </TableCell>
                <TableCell>
                  <Chip
                ...
                <TableCell>
                  <Typography variant="caption">{formatDate(video.moderatedAt)}</Typography>
                </TableCell>
              </TableRow>
            );
          })}
