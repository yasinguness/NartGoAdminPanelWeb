...
export default function OrganizerRow({ organizer, selectedBatches, onToggleBatch, onSelectAllOrganizerBatches }: Props) {
  ...
  const slaColor = oldestHours > 72 ? '#ef4444' : oldestHours > 24 ? '#f59e0b' : oldestHours > 0 ? '#64748b' : 'rgba(30,41,59,0.55)';
    ...
    <Paper variant="outlined" sx={{ borderRadius: 2, bgcolor: '#FFFFFF', borderColor: 'rgba(201,162,39,0.12)', overflow: 'hidden' }}>
      ...
      >
        <Checkbox
          ...
          sx={{ color: 'rgba(30,41,59,0.55)', '&.Mui-checked': { color: '#C9A227' }, '&.MuiCheckbox-indeterminate': { color: '#C9A227' } }}
        />

        <Box sx={{ minWidth: 0, flex: 1 }}>
          <Stack direction="row" spacing={1} alignItems="center">
            <Typography sx={{ fontFamily: 'inherit', fontStyle: 'normal', fontSize: 14, fontWeight: 700, color: '#1E293B', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
        ...
        <IconButton size="small" onClick={(e) => { e.stopPropagation(); setExpanded(v => !v); }} sx={{ color: 'rgba(30,41,59,0.55)' }}>
      ...
      <Collapse in={expanded}>
        <Box sx={{ borderTop: '1px solid rgba(0,0,0,0.05)', bgcolor: 'rgba(0,0,0,0.15)' }}>
          {pendingRefs.length === 0 ? (
            <Typography sx={{ py: 2, px: 3, fontSize: 11, color: 'rgba(30,41,59,0.45)', fontStyle: 'normal' }}>
              Bekleyen batch yok
            </Typography>
          ) : (
            <Table size="small" sx={{ '& .MuiTableCell-root': { borderBottomColor: 'rgba(0,0,0,0.05)', color: '#1E293B', py: 1 } }}>
              <TableHead>
                <TableRow>
...
function BatchRowInner({ batch, currency, selected, onToggle, onOpenSettlement }: {
  ...
  const ageColor = ageHours > 72 ? '#ef4444' : ageHours > 24 ? '#f59e0b' : 'rgba(30,41,59,0.70)';
    ...
    <TableRow hover sx={{ '&:hover': { bgcolor: 'rgba(201,162,39,0.03) !important' } }}>
      <TableCell padding="checkbox">
        <Checkbox
          ...
          checked={selected}
          onChange={onToggle}
          sx={{ color: 'rgba(30,41,59,0.55)', '&.Mui-checked': { color: '#C9A227' } }}
        />
      </TableCell>
      <TableCell sx={{ fontSize: 10, fontFamily: 'monospace', color: 'rgba(30,41,59,0.70)' }}>
      ...
      <TableCell>
        <Stack direction="row" spacing={0.5}>
          ...
          {batch.payoutStatus && batch.payoutStatus !== 'NOT_STARTED' && batch.payoutStatus !== 'UNASSIGNED' && (
            <Chip label={batch.payoutStatus} size="small" sx={{ fontSize: 9, height: 16, bgcolor: 'rgba(0,0,0,0.05)', color: 'rgba(30,41,59,0.60)' }} />
          )}
        </Stack>
      </TableCell>
...
function HeaderCell({ children, align }: { children?: React.ReactNode; align?: 'right' | 'center' }) {
  return (
    <TableCell align={align} sx={{ fontSize: 9, fontWeight: 800, letterSpacing: 1, color: 'rgba(30,41,59,0.55) !important', textTransform: 'uppercase' }}>
      {children}
    </TableCell>
  );
}
