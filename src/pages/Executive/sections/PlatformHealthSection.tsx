    ...
    <SectionCard partial={partial} partialMessage="Platform sağlık metrikleri için /admin/executive/platform-health endpoint'i gerekli. Actuator + Elasticsearch entegrasyonu şart.">
      ...
      <Box sx={{ mt: 3, p: 2.5, borderRadius: 2, bgcolor: 'rgba(0,0,0,0.02)', border: '1px solid rgba(0,0,0,0.05)' }}>
        ...
        {loading ? (
          <Grid container spacing={1}>
            {[1, 2, 3, 4, 5, 6].map(i => (
              <Grid item xs={6} sm={4} md={3} lg={2} key={i}>
                <Skeleton variant="rectangular" height={48} sx={{ bgcolor: 'rgba(0,0,0,0.05)', borderRadius: 1 }} />
              </Grid>
            ))}
          </Grid>
              ...
              <Grid item xs={6} sm={4} md={3} lg={2} key={svc.name}>
                <Box sx={{
                  ...
                  <Box sx={{ minWidth: 0, flex: 1 }}>
                    <Typography sx={{ fontSize: 11, fontWeight: 700, color: '#1E293B', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                      {svc.name}
                    </Typography>
          ...
          <Typography sx={{ fontSize: 12, fontStyle: 'normal', color: 'rgba(30,41,59,0.45)', py: 2, textAlign: 'center' }}>
