  ...
  return (
    <Box sx={{ minHeight: 'calc(100vh - 64px)', bgcolor: '#FAFAFA', color: '#1E293B', mx: { xs: -2, sm: -3 }, my: -3, py: 4 }}>
      <Container maxWidth="xl">
        ...
        {downServices > 0 && (
          <Alert severity="error" icon={false} sx={{ mb: 3, bgcolor: 'rgba(239,68,68,0.08)', border: '1px solid rgba(239,68,68,0.25)', color: '#1E293B' }}>
        ...
        {isLoading ? (
          <Stack spacing={1.5}>
            {[1, 2, 3, 4].map(i => <Skeleton key={i} variant="rectangular" height={60} sx={{ bgcolor: 'rgba(0,0,0,0.03)', borderRadius: 1 }} />)}
          </Stack>
    ...
    >
      <AccordionSummary expandIcon={<ExpandIcon sx={{ color: 'rgba(30,41,59,0.55)' }} />}>
        <Stack direction="row" spacing={2} alignItems="center" sx={{ flex: 1 }}>
          ...
          <Typography sx={{ fontFamily: 'inherit', fontStyle: 'normal', fontSize: 16, fontWeight: 700, color: '#1E293B' }}>
            {service.serviceName}
          </Typography>
          {service.status !== 'up' && (
          ...
          <Stack direction="row" spacing={1.5}>
            ...
            {allJobs.length === 0 && service.status === 'up' && (
              <Typography sx={{ fontSize: 11, color: 'rgba(30,41,59,0.55)', fontStyle: 'normal' }}>job yok</Typography>
            )}
          </Stack>
        </Stack>
      </AccordionSummary>
      <AccordionDetails sx={{ p: 0, borderTop: '1px solid rgba(0,0,0,0.05)' }}>
        {service.error && (
        ...
        {allJobs.length === 0 ? (
          <Box sx={{ py: 3, textAlign: 'center' }}>
            <Typography sx={{ fontSize: 11, color: 'rgba(30,41,59,0.45)', fontStyle: 'normal' }}>
            ...
            <TableBody>
              {allJobs.map((j, idx) => (
                <TableRow key={idx} hover sx={{ '&:hover': { bgcolor: 'rgba(201,162,39,0.03) !important' } }}>
                  ...
                  <TableCell align="right" sx={{ fontSize: 10, fontFamily: 'monospace', color: 'rgba(30,41,59,0.55)' }}>
