  ...
  if (loading || !selectedBatch) {
      return (
          <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '100vh', bgcolor: '#f1f5f9' }}>
              <Typography variant="h6" color="text.secondary">Yükleniyor (Backend Connectivity)...</Typography>
          </Box>
      );
  }
  ...
  return (
    <Box sx={{ display: 'flex', flexDirection: 'column', height: '100%', minHeight: 'calc(100vh - 64px)', bgcolor: '#f1f5f9' }}>
      ...
      <Box sx={{ p: 2, bgcolor: '#ffffff', borderBottom: `1px solid ${COLORS.border}`, display: 'flex', gap: 2, alignItems: 'center', flexWrap: 'wrap' }}>
        <Typography variant="h6" fontWeight={800} sx={{ mr: 2 }}>Settlement & Finance</Typography>
        
        <TextField select size="small" label="Tarih Aralığı (Period)" defaultValue="THIS_WEEK" sx={{ minWidth: 150 }}>
           <MenuItem value="THIS_WEEK">Bu Hafta</MenuItem>
           <MenuItem value="LAST_WEEK">Geçen Hafta</MenuItem>
        </TextField>
        <TextField select size="small" label="Organizer" defaultValue="ALL" sx={{ minWidth: 200 }}>
           <MenuItem value="ALL">Tüm Organizatörler</MenuItem>
           <MenuItem value="O1">Zorlu PSM</MenuItem>
        </TextField>
        <TextField select size="small" label="Durum (Status)" defaultValue="ALL" sx={{ minWidth: 150 }}>
           <MenuItem value="ALL">Tümü</MenuItem>
           <MenuItem value="READY">Ready for Payout</MenuItem>
           <MenuItem value="ON_HOLD">On Hold</MenuItem>
        </TextField>
        
        <Box sx={{ flexGrow: 1 }} />
        <Button variant="outlined" startIcon={<ExportIcon />} sx={{ fontWeight: 700, borderColor: COLORS.border, color: '#334155' }}>Toplu Export</Button>
      </Box>
      ...
      <Box sx={{ p: 2, bgcolor: '#0f172a', color: 'white', position: 'sticky', top: 64, zIndex: 10, display: 'flex', justifyContent: 'space-between', gap: 2, overflowX: 'auto', borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
        <KpiItem label="Gross Sales" value="₺470,000" subValue="Konsolide Satış" color="#38bdf8" />
        <Divider orientation="vertical" flexItem sx={{ borderColor: 'rgba(255,255,255,0.1)' }} />
        <KpiItem label="Net Sales" value="₺411,000" subValue="İadeler Düşülmüş" color="#10b981" />
        <Divider orientation="vertical" flexItem sx={{ borderColor: 'rgba(255,255,255,0.1)' }} />
        <KpiItem label="Refund Total" value="- ₺59,000" subValue="%12.5 Refund Oranı" color="#ef4444" />
        <Divider orientation="vertical" flexItem sx={{ borderColor: 'rgba(255,255,255,0.1)' }} />
        <KpiItem label="Service Fee Rev" value="+ ₺23,500" subValue="Platform Geliri" color="#8b5cf6" />
        <Divider orientation="vertical" flexItem sx={{ borderColor: 'rgba(255,255,255,0.1)' }} />
        <Box sx={{ display: 'flex', flexDirection: 'column', bgcolor: 'rgba(16, 185, 129, 0.1)', px: 3, py: 0.5, borderRadius: 2, border: '1px solid rgba(16, 185, 129, 0.3)' }}>
            <Typography variant="caption" sx={{ color: '#94a3b8', fontWeight: 700 }}>ORGANIZER PAYABLE</Typography>
      ...
      <Box sx={{ display: 'flex', flex: 1, overflow: 'hidden' }}>
        ...
        <Box sx={{ width: '28%', minWidth: 320, borderRight: `1px solid ${COLORS.border}`, bgcolor: 'white', display: 'flex', flexDirection: 'column' }}>
            <Box sx={{ p: 2, borderBottom: `1px solid ${COLORS.border}`, bgcolor: COLORS.surface }}>
                <Typography variant="subtitle2" fontWeight={800} color="text.secondary">HAKEDİŞ KUYRUĞU (BATCHES)</Typography>
            </Box>
                    ...
                    >
                        ...
                        <Stack direction="row" justifyContent="space-between" sx={{ width: '100%' }}>
                            ...
                            <Box sx={{ textAlign: 'right' }}>
                                <Typography variant="caption" color="text.secondary" display="block" fontWeight={700}>NET PAYABLE</Typography>
        ...
        <Box sx={{ width: '42%', minWidth: 500, bgcolor: COLORS.surface, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
            ...
            <Box sx={{ p: 4, bgcolor: 'white', borderBottom: `1px solid ${COLORS.border}` }}>
                <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', mb: 2 }}>
                    ...
                    <Box sx={{ textAlign: 'right' }}>
                        <Typography variant="caption" color="text.secondary" fontWeight={700} display="block">PAYOUT STATUS</Typography>
                ...
                {selectedBatch.anomalyFlags.length > 0 && (
                     <Paper elevation={0} sx={{ p: 2, bgcolor: '#fef2f2', border: `1px solid ${COLORS.error}`, borderRadius: 2, mb: 2 }}>
                         <Typography variant="subtitle2" color={COLORS.error} fontWeight={800} sx={{ display: 'flex', alignItems: 'center' }}>
                             <WarningIcon sx={{ mr: 1 }} /> Anomaly (Risk) Algılandı: Payout Durduruldu
                         </Typography>
            ...
            <Box sx={{ p: 4, flex: 1, overflow: 'auto' }}>
                <Typography variant="subtitle2" fontWeight={800} color="text.secondary" sx={{ mb: 2, letterSpacing: 1 }}>NET PAYABLE FORMULA (DECOMPOSITION)</Typography>
                
                <Paper elevation={0} sx={{ p: 3, borderRadius: 2, border: `1px solid ${COLORS.border}`, display: 'flex', flexDirection: 'column', gap: 1.5 }}>
                    <FormulaRow label="Brüt Satışlar (Gross Sales)" val={selectedBatch.grossSales} type="POSITIVE" />
                    <FormulaRow label="İadeler (Refunds)" val={selectedBatch.refundedAmount} type="NEGATIVE" />
                    ...
                    <Box sx={{ height: 16 }} />
                    <FormulaRow label="Platform Komisyonu (Fees)" val={selectedBatch.serviceFeeRevenue} type="NEGATIVE" />
                    <FormulaRow label="Vergiler (Taxes / Withholding)" val={selectedBatch.taxTotal} type="NEGATIVE" />
                    <FormulaRow label="Chargeback Kesintisi" val={selectedBatch.chargebackTotal} type="NEGATIVE" />
                    <FormulaRow label="Manuel Adjustments (Goodwill vs)" val={selectedBatch.manualAdjustmentsTotal} type={selectedBatch.manualAdjustmentsTotal < 0 ? 'NEGATIVE' : 'POSITIVE'} />

                    <Divider sx={{ my: 1, borderBottomWidth: 2, borderColor: '#cbd5e1' }} />
                    
                    <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                        <Typography variant="subtitle1" fontWeight={800} color="#0f172a">ORGANIZER NET PAYABLE</Typography>
            ...
            <Box sx={{ flex: 1, overflow: 'auto', p: 0 }}>
                        ...
                        {mockLedger.map(l => (
                            <ListItemButton key={l.id} sx={{ borderBottom: `1px solid ${COLORS.surface}`, p: 2 }}>
                                <Box sx={{ flex: 1 }}>
                                    ...
                                    <Typography variant="caption" color="text.secondary" sx={{ fontFamily: 'monospace' }}>Type: {l.type}</Typography>
                                </Box>
                ...
                {rightTab === 1 && (
                    <Box sx={{ p: 3, textAlign: 'center' }}>
                        ...
                        <Typography variant="subtitle2" fontWeight={700}>Manuel Adjustment Ekle</Typography>
                        ...
                        <Button variant="outlined" sx={{ fontWeight: 700 }}>+ YENİ DÜZELTME (DEBIT/CREDIT)</Button>
                    </Box>
                )}
                ...
                {rightTab === 2 && (
                    <Box sx={{ p: 3, display: 'flex', flexDirection: 'column', gap: 2 }}>
                        ...
                        <Button variant="outlined" startIcon={<ExportIcon/>} sx={{ justifyContent: 'flex-start', py: 1.5, fontWeight: 700 }}>Export Settlement (XLSX)</Button>
                        <Button variant="outlined" startIcon={<ExportIcon/>} sx={{ justifyContent: 'flex-start', py: 1.5, fontWeight: 700 }}>Download Statement (PDF)</Button>
                        <Button variant="outlined" startIcon={<BankIcon/>} sx={{ justifyContent: 'flex-start', py: 1.5, fontWeight: 700 }}>Muhasebe ERP Entegrasyonu (Push)</Button>
                    </Box>
                )}
            </Box>
        </Box>
